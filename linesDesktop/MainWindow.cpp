...
MainWindow::MainWindow() : m_progressDialog(0) {
  ...
  connect(m_networkManager, SIGNAL(StateChangedSignal()), this,
  ...
  // Create main widget  (just a frame to put the viewing widget and the
  ...
  // Set default OpenGL surface format that will be applied to all openGL
  // windows
  ...
  m_playbackControlerWidget =
      new PlaybackControlerWidget(m_lines, m_mainWidget);
  ...
  m_dockTabletState->setFeatures(QDockWidget::DockWidgetClosable |
                                 QDockWidget::DockWidgetMovable);

  // Read Application settings
  readSettings();

  // Tell the qApp unique instance to send event to MainWindow::eventFilter
  // before anyone else so that we can grab global keyboard shortcuts.
...
bool MainWindow::IsSideToolbarShown() { return !m_sideToolbar->isHidden(); }
...
bool MainWindow::IsSideToolbarLeft() { return m_sideToolbarLeft; }
...
void MainWindow::CreateActions() {
  ...
  m_editSetNumberOfFramesAction = m_editMenu->addAction(
      "Set Number of Frames", this, SLOT(editSetNumberOfFrames()),
      Qt::CTRL + Qt::Key_G);
  ...
  // connect( m_whiteOnBlackAction, SIGNAL(toggled(bool)), this,
  // SLOT(editWhiteOnBlackToggled(bool)) ); m_editMenu->addAction(
  // m_whiteOnBlackAction );
  connect(m_editMenu, SIGNAL(aboutToShow()), this, SLOT(editMenuAboutToShow()));
  QAction *preferencesAction =
      m_editMenu->addAction("preferences...", this, SLOT(editPreferences()));

  // Create the Network menu
  m_networkMenu = menuBar()->addMenu("&Network");
  m_netShareSessionMenuAction = m_networkMenu->addAction(
      "Share session", this, SLOT(NetShareSession()), Qt::CTRL + Qt::Key_T);
  m_netConnectMenuItem =
      m_networkMenu->addAction("Connect...", this, SLOT(NetConnect()));

  // Create the View menu
  m_viewMenu = menuBar()->addMenu("&View");
#ifdef Q_OS_WIN
  m_viewMenu->addAction("Fullscreen", this, SLOT(viewFullscreen()),
                        Qt::CTRL + Qt::Key_F);
#endif // Q_OS_WIN
...
void MainWindow::fileMenuAboutToShow() {
  bool networkOn =
      m_networkManager->IsConnected() || m_networkManager->IsSharing();
...
void MainWindow::fileOpen() {
  ...
  // Get the filename
  m_filename = QFileDialog::getOpenFileName(this, tr("Open Animation"),
                                            m_fileDialogStartPath,
                                            tr("Animation Files (*.drw)"));
...
bool MainWindow::fileExport() {
  ExportDialog dlg(m_exportDefaultPath, m_exportRes.width(),
                   m_exportRes.height());
  if (dlg.exec() == QDialog::Accepted) {
    ...
    if (res) {
      ...
      connect(exporter, SIGNAL(WritingFrame(int)), &dialog, SLOT(setValue(int)),
              Qt::QueuedConnection);
      dialog.exec();
...
void MainWindow::editSetNumberOfFrames() {
  int nbFrames =
      QInputDialog::getInt(this, "Set Number of Frames", "Number of Frames",
                           m_lines->GetNumberOfFrames(), 0);
...
void MainWindow::editPreferences() {
  if (!m_linesPreferences) {
    ...
    connect(m_linesPreferences, SIGNAL(destroyed(QObject *)), this,
            SLOT(OnLinesPreferencesClosed()));
    m_linesPreferences->SetMainWindow(this);
  }
  m_linesPreferences->show();
}

void MainWindow::OnLinesPreferencesClosed() { m_linesPreferences = 0; }
...
void MainWindow::NetStateChanged() {
  ...
  if (state == drwNetworkManager::ConnectionLost) {
    ...
    QMessageBox::warning(this, "Server not responding",
...
void MainWindow::SetWhiteOnBlack(bool wob) {
  ...
  Vec4 newFgCol(1.0 - currentFgCol[0], 1.0 - currentFgCol[1],
                1.0 - currentFgCol[2], currentFgCol[3]);
...
bool MainWindow::GetSaveFilename() {
  m_filename = QFileDialog::getSaveFileName(
      ...
      m_fileDialogStartPath + QDir::separator() + "untitled.drw",
      tr("Animation Files (*.drw)"));
  if (m_filename.isEmpty())
    return false;
  m_fileDialogStartPath = m_filename;
...
bool MainWindow::maybeSave() {
  if (m_lines->IsAnimationModified()) {
    ...
    ret = QMessageBox::warning(this, m_appName,
...
void MainWindow::readSettings() {
  ...
  QPoint pos =
      settings.value("MainWindow_pos", mainWindowRect.topLeft()).toPoint();
  QSize size =
      settings.value("MainWindow_size", mainWindowRect.size()).toSize();
  resize(size);
  move(pos);
  ...
  // Save path
  m_fileDialogStartPath =
      settings.value("filedialogstartpath", QDir::homePath()).toString();

  // Export settings
  m_exportDefaultPath =
      settings.value("ExportDefaultPath", QDir::homePath()).toString();
  ...
  // Last server address
  m_lastServerAddress = QHostAddress(
      settings.value("LastServerAddress", QHostAddress().toString())
          .toString());
...
void MainWindow::about() {
  QString msg = QString("Lines - version %1.%2.%3\n")
                    .arg(LINES_MAJOR_VERSION)
                    .arg(LINES_MINOR_VERSION)
                    .arg(LINES_PATCH_VERSION);
...
void MainWindow::ToggleShowGlobalLineParams() {
  if (m_globalLineParamsDock) {
    ...
    m_globalLineParamsDock->setFeatures(QDockWidget::DockWidgetClosable |
                                        QDockWidget::DockWidgetFloatable);
    m_globalLineParamsDock->setAttribute(Qt::WA_DeleteOnClose);
    connect(m_globalLineParamsDock, SIGNAL(destroyed()), this,
            SLOT(GlobalLineParamsDockClosed()));
    m_globalLineParamsDock->show();
  }
}

void MainWindow::GlobalLineParamsDockClosed() { m_globalLineParamsDock = 0; }
