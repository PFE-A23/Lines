...
LinesPreferencesWidget::LinesPreferencesWidget(QWidget *parent)
    : QWidget(parent), ui(new Ui::LinesPreferencesWidget) {
  m_mainWindow = 0;
  ui->setupUi(this);
}

LinesPreferencesWidget::~LinesPreferencesWidget() { delete ui; }
...
void LinesPreferencesWidget::UpdateUI() {
  ...
  ui->sideToolbarRightRadioButton->setChecked(
      !m_mainWindow->IsSideToolbarLeft());
...
void LinesPreferencesWidget::on_sideToolbarLeftRadioButton_toggled(
    bool checked) {
  if (checked)
    m_mainWindow->SetSideToolbarLeft(true);
}

void LinesPreferencesWidget::on_sideToolbarRightRadioButton_toggled(
    bool checked) {
  if (checked)
...
void LinesPreferencesWidget::on_mainToolbarHeightSpinBox_valueChanged(
    int arg1) {
