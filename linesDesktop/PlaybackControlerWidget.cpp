...
PlaybackControlerWidget::PlaybackControlerWidget(LinesCore *lc, QWidget *parent)
    : QWidget(parent), m_lines(lc), playIcon("://icons-60x60/play.png"),
      pauseIcon("://icons-60x60/pause.png") {
  SetupUi();
  UpdateUi();

  connect(m_lines, SIGNAL(PlaybackStartStop(bool)), this,
          SLOT(PlaybackStartStopSlot(bool)));
  connect(m_lines, SIGNAL(PlaybackSettingsChangedSignal()), this,
          SLOT(UpdateUi()));
}
...
void PlaybackControlerWidget::PlayPauseButtonClicked() { m_lines->PlayPause(); }
...
void PlaybackControlerWidget::SetupUi() {
  ...
  // Current frame line edit
  {
    ...
    sizePolicy.setHeightForWidth(
        currentFrameLineEdit->sizePolicy().hasHeightForWidth());
  ...
  // Space
  postLoopSpacer =
      new QSpacerItem(28, 20, QSizePolicy::Fixed, QSizePolicy::Minimum);
  ...
  // Frame rate slider
  {
    ...
    sizePolicy.setHeightForWidth(
        frameRateSlider->sizePolicy().hasHeightForWidth());
  ...
  // Frame rate line edit
  {
    ...
    QSizePolicy sizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
    sizePolicy.setHeightForWidth(
        frameRateLineEdit->sizePolicy().hasHeightForWidth());
  ...
  connect(currentFrameSlider, SIGNAL(valueChanged(int)), this,
          SLOT(OnCurrentFrameSliderValueChanged(int)));
  connect(frameRateSlider, SIGNAL(valueChanged(int)), this,
          SLOT(OnFrameRateSliderValueChanged(int)));
