...
inline Real *Vec2::Ref() const { return ((Real *)elt); }
...
inline Real len(const Vec2 &v) { return (sqrt(dot(v, v))); }

inline Real sqrlen(const Vec2 &v) { return (dot(v, v)); }
...
inline Void normalise(Vec2 &v) { v /= len(v); }
...
inline Vec2::Vec2(Axis k) { MakeUnit(k, vl_one); }
