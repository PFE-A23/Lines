...
inline Real *Vec4::Ref() const { return ((Real *)elt); }
...
inline Vec4::Vec4(ZeroOrOne k) { MakeBlock(k); }

inline Vec4::Vec4(Axis k) { MakeUnit(k, vl_1); }
...
inline Real len(const Vec4 &v) { return (sqrt(dot(v, v))); }

inline Real sqrlen(const Vec4 &v) { return (dot(v, v)); }
...
inline Void normalise(Vec4 &v) { v /= len(v); }
