...
Mat3::Mat3(Real a, Real b, Real c, Real d, Real e, Real f, Real g, Real h,
           Real i) {
  row[0][0] = a;
  row[0][1] = b;
  row[0][2] = c;
...
Real trace(const Mat3 &m) { return (m[0][0] + m[1][1] + m[2][2]); }

Real det(const Mat3 &m) { return (dot(m[0], cross(m[1], m[2]))); }
