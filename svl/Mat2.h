...
inline Real *Mat2::Ref() const { return ((Real *)row); }
...
inline Mat2::Mat2(ZeroOrOne k) { MakeDiag(k); }

inline Mat2::Mat2(Block k) { MakeBlock((ZeroOrOne)k); }
...
inline Real trace(const Mat2 &m) { return (m[0][0] + m[1][1]); }
...
#ifdef VL_ROW_ORIENT
inline Vec2 xform(const Mat2 &m, const Vec2 &v) { return (v * m); }
inline Mat2 xform(const Mat2 &m, const Mat2 &n) { return (n * m); }
#else
inline Vec2 xform(const Mat2 &m, const Vec2 &v) { return (m * v); }
inline Mat2 xform(const Mat2 &m, const Mat2 &n) { return (m * n); }
#endif
