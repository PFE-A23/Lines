...
inline Real *Vec3::Ref() const { return ((Real *)elt); }
...
inline Vec3::Vec3(Axis a) { MakeUnit(a, vl_one); }
...
inline Real len(const Vec3 &v) { return (sqrt(dot(v, v))); }

inline Real sqrlen(const Vec3 &v) { return (dot(v, v)); }
...
inline Void normalise(Vec3 &v) { v /= len(v); }
