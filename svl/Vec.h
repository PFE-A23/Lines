...
inline Int Vec::Elts() const { return (elts & VL_REF_MASK); }
...
inline Real *Vec::Ref() const { return (data); }
...
inline Real len(const Vec &v) { return (sqrt(dot(v, v))); }

inline Real sqrlen(const Vec &v) { return (dot(v, v)); }
...
inline Void normalise(Vec &v) { v /= len(v); }
