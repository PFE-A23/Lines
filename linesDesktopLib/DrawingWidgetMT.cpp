...
void RenderThread::SetDrawingWidget(DrawingWidgetMT *c) { m_drawingWidget = c; }

void RenderThread::SetLinesCore(LinesCore *lc) { m_lines = lc; }
...
DrawingWidgetMT::DrawingWidgetMT(LinesCore *lc, QWidget *parent)
    : QOpenGLWidget(parent), m_lines(lc), m_timerId(-1), m_muteMouse(false),
  ...
  connect(this, SIGNAL(aboutToCompose()), this, SLOT(startCompositing()),
          Qt::DirectConnection);
  connect(this, SIGNAL(frameSwapped()), this, SLOT(finishCompositing()),
          Qt::DirectConnection);
  connect(this, SIGNAL(aboutToResize()), this, SLOT(StartResize()),
          Qt::DirectConnection);
  connect(this, SIGNAL(resized()), this, SLOT(FinishResize()),
          Qt::DirectConnection);
  connect(this, SIGNAL(PostRenderSignal()), this, SLOT(PostRenderSlot()),
          Qt::QueuedConnection);
...
void DrawingWidgetMT::NotifyPlaybackStartStop(bool isStarting) {
  // Start/stop generating idle events that are used to make sure we redraw
  // during playback
  if (isStarting) {
...
void DrawingWidgetMT::WaitRenderFinished() { m_renderThread->wait(); }
...
void DrawingWidgetMT::PostRender() { emit PostRenderSignal(); }
...
void DrawingWidgetMT::finishCompositing() {
  // buffer has just been swapped, we can start rendering again if there are
  // pending lines to draw
  TryRender();
}
...
void DrawingWidgetMT::PostRenderSlot() { TryRender(); }

void DrawingWidgetMT::initializeGL() {
  // cout << "Support threaded opengl: " <<
  // (this->context()->supportsThreadedOpenGL() ? "true" : "false") << endl;
}
...
void DrawingWidgetMT::MouseCommand(
    drwMouseCommand::MouseCommandType commandType, QMouseEvent *e) {
  int ratio = this->devicePixelRatio();
...
void DrawingWidgetMT::MouseCommand(
    drwMouseCommand::MouseCommandType commandType, QTabletEvent *e) {
  int ratio = this->devicePixelRatio();
  ...
  m_lines->MouseEvent(commandType, xWin, yWin, e->pressure(), e->xTilt(),
                      e->yTilt(), e->rotation(), e->tangentialPressure());
  TryRender();
}
