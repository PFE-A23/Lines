...
public:
  ...
  // Functions used to control rendering. For direct render, call
  // WaitRenderFinished, then do whatever is needed to the scene and renderer
  // and then call Render. Render assumes that render thread is not running,
  // i.e., that wait has been called before.
...
protected:
  ...
  void MouseCommand(drwMouseCommand::MouseCommandType commandType,
                    QMouseEvent *e);
  void MouseCommand(drwMouseCommand::MouseCommandType commandType,
                    QTabletEvent *e);
