...
drwDrawingEngine::drwDrawingEngine()
    : m_isRunning(false), m_nbPointsInSegment(0), m_lastPoint(Vec2(0, 0)),
      m_lastAngle(0.0), m_lastAngularSpeed(0.0),
      m_lastAngularAcceleration(0.0) {}

void drwDrawingEngine::Start() {
  if (!m_isRunning) {
    connect(m_drawingWidget, SIGNAL(FinishedPainting()), this, SLOT(Tick()),
            Qt::QueuedConnection);
    m_isRunning = true;
    Tick();
  }
...
void drwDrawingEngine::Tick() {
  ...
  if (--m_nbPointsInSegment < 0) {
    ...
    m_lastAngularAcceleration +=
        RandomDouble(-m_maxAngularAcceleration, m_maxAngularAcceleration);
...
// the range [0,1].
double drwDrawingEngine::RandomDouble() { return vl_rand(); }
