...
drwLineToolViewportWidget::drwLineToolViewportWidget(drwDrawingWidget *w,
                                                     drwLineTool *lt)
    : m_x(0), m_y(0), m_active(false), m_changingColor(false),
      m_changingSize(false), m_lastX(0), m_lastY(0), m_drawingWidget(w),
      m_lineTool(lt) {}
...
void drwLineToolViewportWidget::Activate(int x, int y) {
  ...
  m_boundingRect.setRect(boundingCorner[0], boundingCorner[1], boundingSize[0],
                         boundingSize[1]);
}
...
void drwLineToolViewportWidget::Draw(QPainter &painter) {
  if (m_active) {
    ...
    painter.drawRoundedRect(m_boundingRect, m_roundedCornerRadius,
                            m_roundedCornerRadius);

    // Draw scaling rect
    // painter.setBrush( QBrush( Qt::gray ) );
    painter.setBrush(QBrush(scaleWidgetColor));
    painter.drawRoundedRect(m_scaleRect, m_roundedCornerRadius,
                            m_roundedCornerRadius);
    ...
    // painter.drawEllipse( QPoint( plusX, plusY ), m_scaleButtonRadius,
    // m_scaleButtonRadius );
    painter.setBrush(QBrush(Qt::white));
    painter.drawRect(plusX - m_scaleSignRadius, plusY - 1,
                     2 * m_scaleSignRadius, 3);
    painter.drawRect(plusX - 1, plusY - m_scaleSignRadius, 3,
                     2 * m_scaleSignRadius);
    ...
    // painter.drawEllipse( QPoint( minusX, minusY), m_scaleButtonRadius,
    // m_scaleButtonRadius );
    painter.setBrush(QBrush(Qt::white));
    painter.drawRect(minusX - m_scaleSignRadius, minusY - 1,
                     2 * m_scaleSignRadius, 3);

    // Draw gradient
    QLinearGradient linearGrad(QPointF(0, m_gradRect.y() + m_height),
                               QPointF(0, m_gradRect.y()));
    linearGrad.setColorAt(0, Qt::black);
    linearGrad.setColorAt(1, Qt::white);
    painter.setBrush(QBrush(linearGrad));
    painter.drawRoundedRect(m_gradRect, m_roundedCornerRadius,
                            m_roundedCornerRadius);
