...
drwDrawingWidget::drwDrawingWidget(QWidget *parent)
    : QOpenGLWidget(parent)
      ...
      ,
      m_isPlaying(false), m_muteMouse(false), m_tabletHasControl(false),
      m_lines(0) {
  m_timerId = -1;
  // PrintGLInfo();
  ...
  setUpdateBehavior(
      QOpenGLWidget::PartialUpdate); // allows to redraw only part of the window
  EnableVSync(false);
}

drwDrawingWidget::~drwDrawingWidget() {}

void drwDrawingWidget::NeedRedraw() { update(); }
...
void drwDrawingWidget::enterEvent(QEvent *e) { m_lines->SetShowCursor(true); }

void drwDrawingWidget::leaveEvent(QEvent *e) { m_lines->SetShowCursor(false); }

bool drwDrawingWidget::event(QEvent *e) {
  // Give a chance to the controler to change frame. A frame change triggers an
  // updateGL
  if (m_isPlaying)
    m_lines->Tick();
...
void drwDrawingWidget::PrintGLInfo() {
  ...
  // OpenGL version
  std::cout << "OpenGL version: " << f.majorVersion() << "." << f.minorVersion()
            << std::endl;
...
void drwDrawingWidget::MouseCommand(
