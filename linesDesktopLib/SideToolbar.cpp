...
SideToolbar::SideToolbar(QWidget *parent)
    : QWidget(parent), ui(new Ui::SideToolbar) {
  ui->setupUi(this);
  connect(ui->scrollWidget, SIGNAL(wheelClick(int)), this,
          SLOT(OnScrollWidgetClick(int)));
}

SideToolbar::~SideToolbar() { delete ui; }
...
void SideToolbar::on_nextFrameButton_clicked() { m_app->GotoNextFrame(); }

void SideToolbar::on_prevFrameButton_clicked() { m_app->GotoPreviousFrame(); }
