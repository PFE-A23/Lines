...
Receiver::Receiver(QWidget *parent) : QDialog(parent), m_connection(NULL) {
  ...
  connect(m_networkInterface, SIGNAL(NewConnection(drwNetworkConnection *)),
          this, SLOT(NewConnection(drwNetworkConnection *)));
  connect(m_networkInterface, SIGNAL(ConnectionLost(drwNetworkConnection *)),
...
void Receiver::NewConnection(drwNetworkConnection *connection) {
  ...
  connect(connection, SIGNAL(CommandReceived(drwCommand::s_ptr)), this,
          SLOT(CommandReceived(drwCommand::s_ptr)));
