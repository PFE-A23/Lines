...
drwGlslShader::drwGlslShader()
    : m_glslShader(0), m_glslVertexShader(0), m_glslProg(0), m_init(false) {}

drwGlslShader::~drwGlslShader() { Clear(); }
...
bool drwGlslShader::Init() {
  ...
  if (m_vertexMemSources.size() != 0 || m_vertexShaderFilenames.size() != 0)
    if (!CreateAndCompileShader(GL_VERTEX_SHADER, m_glslVertexShader,
                                m_vertexShaderFilenames, m_vertexMemSources))
      return false;

  // Load and try compiling pixel shader
  if (m_memSources.size() != 0 || m_shaderFilenames.size() != 0)
    if (!CreateAndCompileShader(GL_FRAGMENT_SHADER, m_glslShader,
                                m_shaderFilenames, m_memSources))
      return false;
...
bool drwGlslShader::CreateAndCompileShader(
    unsigned shaderType, unsigned &shaderId, std::vector<std::string> &files,
...
bool drwGlslShader::LoadOneShaderSource(const char *filename,
