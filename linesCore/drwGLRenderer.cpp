...
void drwGLRenderer::Render() {
  ...
  if (m_sceneModified) {
    ...
    RenderToTexture(m_renderFrame, onionSkinBefore, onionSkinAfter,
                    modifiedRectWin);
...
void drwGLRenderer::RenderToTexture(int currentFrame, int onionSkinBefore,
                                    int onionSkinAfter, Box2i &rect) {
  Q_ASSERT(m_scene);
  ...
  // Only update specified region. rect is specified in window coordinate
  // system, we need to change it to frame (texture) coordinate system.
  glEnable(GL_SCISSOR_TEST);
  Box2i frameRect;
  GLWindowToGLFrame(rect, frameRect);
  glScissor(frameRect.XMin(), frameRect.YMin(), frameRect.GetWidth(),
            frameRect.GetHeight());
...
int *drwGLRenderer::GetRenderSize() { return m_camera->GetWindowSize(); }

void drwGLRenderer::WindowToWorld(double xWin, double yWin, double &xWorld,
                                  double &yWorld) {
...
void drwGLRenderer::GLWindowToWorld(double xWin, double yWin, double &xWorld,
                                    double &yWorld) {
...
void drwGLRenderer::GLWindowToGLFrame(int xwin, int ywin, int &xframe,
                                      int &yframe) {
...
void drwGLRenderer::WorldToGLFrame(double xworld, double yworld, int &xwin,
                                   int &ywin) {
...
void drwGLRenderer::WorldToGLWindow(double xworld, double yworld, int &xwin,
                                    int &ywin) {
...
double drwGLRenderer::PixelsPerUnit() { return m_camera->PixelsPerUnit(); }

double drwGLRenderer::UnitsPerPixel() { return m_camera->UnitsPerPixel(); }
...
void drwGLRenderer::NeedRedraw() {
  m_sceneModified = true;
  Box2d modifiedRect(0.0, m_camera->GetFrameSize()[0], 0.0,
                     m_camera->GetFrameSize()[1]);
