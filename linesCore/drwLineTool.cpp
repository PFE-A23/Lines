...
drwLineTool::drwLineTool(Scene *scene, drwToolbox *toolbox)
    : drwTool(scene, toolbox), m_lastXWorld(0.0), m_lastYWorld(0.0),
      m_lastPressure(1.0), m_isDrawing(false), m_tabletHasControl(false),
      Color(1.0, 1.0, 1.0, 1.0), Type(TypeWideLine), m_brushWidth(5.0),
      m_eraseWidth(20.0), m_baseWidth(5.0), m_pressureWidth(true),
      m_pressureOpacity(true), m_fill(false), m_erase(false),
      m_frameChangeMode(Manual), m_persistenceEnabled(false), m_persistence(0),
      m_minWidth(2.0), m_maxWidth(100.0), m_cursor(0) {
  if (IsLocal()) {
    m_cursor = new Circle;
...
void drwLineTool::ReadSettings(QSettings &s) {
  ...
  m_pressureWidth =
      s.value("PressureWidth", QVariant(m_pressureWidth)).toBool();
  m_pressureOpacity =
      s.value("PressureOpacity", QVariant(m_pressureOpacity)).toBool();
...
void drwLineTool::OnStartPlaying() { SetPersistenceEnabled(true); }

void drwLineTool::OnStopPlaying() { SetPersistenceEnabled(false); }

void drwLineTool::StartLine(double xWorld, double yWorld, double pressure) {
  drwCommand::s_ptr command(
      new drwMouseCommand(drwMouseCommand::Press, xWorld, yWorld, (int)xWorld,
                          (int)yWorld, 0.0, 0, 0, pressure, 0.0, 0.0));
  ExecuteCommand(command);
}

void drwLineTool::AddPoint(double xWorld, double yWorld, double pressure) {
  drwCommand::s_ptr command(
      new drwMouseCommand(drwMouseCommand::Move, xWorld, yWorld, (int)xWorld,
                          (int)yWorld, 0.0, 0, 0, pressure, 0.0, 0.0));
  ExecuteCommand(command);
}
...
void drwLineTool::EndLine(double xWorld, double yWorld, double pressure) {
  drwCommand::s_ptr command(
      new drwMouseCommand(drwMouseCommand::Release, xWorld, yWorld, (int)xWorld,
                          (int)yWorld, 0.0, 0, 0, pressure, 0.0, 0.0));
  ExecuteCommand(command);
}
...
void drwLineTool::ExecuteLineToolParamCommand(drwCommand::s_ptr command) {
  drwLineToolParamsCommand *paramCom =
      dynamic_cast<drwLineToolParamsCommand *>(command.get());
  Q_ASSERT(paramCom);
...
void drwLineTool::EmitStateCommand() { ParametersChanged(); }
...
bool drwLineTool::IsPresureWidthEnabled() { return !GetFill(); }
...
double drwLineTool::GetBaseWidth() { return m_baseWidth; }
...
Node *drwLineTool::CreateNewNode(double x, double y, double pressure) {
  ...
  case TypeWideLine: {
    ...
    newWideLine->SetPressureOpacity(IsPresureOpacityEnabled() &&
                                    m_pressureOpacity);
...
void drwLineTool::MarkSegmentModified(double x1, double y1, double x2,
                                      double y2) {
