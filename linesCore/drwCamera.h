...
public:
  ...
  void GetFrameSizePix(int frameSize[2]) {
    frameSize[0] = m_frameSizePix[0];
    frameSize[1] = m_frameSizePix[1];
  }
...
private:
  void UpdateSizes();
  void RectThatFitsInside(double w, double h, double percentBorder, int winW,
                          int winH, int &rectW, int &rectH);
  ...
  int m_framePosPix[2];
  double m_winPosWorld[2];  // position of the bottom left corner of the window
                            // in world space
