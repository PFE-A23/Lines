...
void Circle::SetCenter(Vec2 center) { SetCenter(center[0], center[1]); }
...
const Box2d &Circle::BoundingBox() {
  m_boundingBox.Init(Center[0] - Radius, Center[0] + Radius, Center[1] - Radius,
                     Center[1] + Radius);
...
void Circle::InternDraw(drwDrawingContext &context) {
  ...
  if (points.size() > 0) {
    ...
    if (triangles.size() > 0) {
      ...
      glDrawElements(GL_TRIANGLES, numberOfIndexes, GL_UNSIGNED_INT,
                     triangles.GetBuffer());
    }
    ...
    if (lines.size() > 0) {
      ...
      glDrawElements(GL_LINE_STRIP, numberOfIndexes, GL_UNSIGNED_INT,
                     lines.GetBuffer());
    }
  }
}
...
void Circle::MarkModified() { m_poly.Clear(); }
