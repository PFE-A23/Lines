...
public:
  ...
  Scene *GetScene() {
    return m_scene;
  } // needed by exporter. todo : fix this in the future
  ...
  // Rendering
  void Render();
  void NotifyNeedRender(); // only for drwGlobalLineParams. todo: We could
                           // inside LinesCore
  void EnableRendering(
      bool enable); // needed to block rendering when receiving an animation.
                    // todo: use same mechanism for loading and receiving

  // Local drawing
  void MouseEvent(drwMouseCommand::MouseCommandType commandType, double xPix,
                  double yPix, double pressure = 1.0, int xTilt = 0,
                  int yTilt = 0, double rotation = 0.0,
                  double tangentialPressure = 0.0);
  void MouseEventWorld(drwMouseCommand::MouseCommandType type, double xWorld,
                       double yWorld, double pressure);
...
private:
  ...
  void HandleFlipFrameEvent(drwMouseCommand::MouseCommandType commandType,
                            double yWin);
