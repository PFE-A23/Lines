...
enum drwCommandId {
  drwIdSetFrameCommand,
  drwIdMouseCommand,
  drwIdLineToolParamsCommand,
  drwIdServerInitialCommand,
  drwIdNewSceneCommand,
  drwIdSceneParamsCommand
};
...
public:
  ...
  // Reading ( the XXXSize() functions are used by clients to determine if there
  // is enough data in a stream to read commands)
...
public:
  ...
  drwSetFrameCommand(drwSetFrameCommand &other)
      : drwCommand(other), NewFrame(other.NewFrame) {}
  virtual ~drwSetFrameCommand() {}
  virtual s_ptr Clone() {
    s_ptr newCom(new drwSetFrameCommand(*this));
    return newCom;
  }
...
public:
  ...
  virtual s_ptr Clone() {
    s_ptr newCom(new drwLineToolParamsCommand(*this));
    return newCom;
  }
...
public:
  ...
  drwServerInitialCommand(drwServerInitialCommand &other)
      : drwCommand(other), ProtocolVersion(other.ProtocolVersion),
        NumberOfCommands(other.NumberOfCommands) {}
  virtual ~drwServerInitialCommand() {}
  virtual s_ptr Clone() {
    s_ptr newCom(new drwServerInitialCommand(*this));
    return newCom;
  }
...
public:
  ...
  drwMouseCommand();
  drwMouseCommand(MouseCommandType commandType, double x, double y, double z,
                  int xPix, int yPix, int xTilt, int yTilt, double pressure,
                  double rotation, double tangentialPressure);
  drwMouseCommand(drwMouseCommand &);
  virtual ~drwMouseCommand() {}
  virtual s_ptr Clone() {
    s_ptr newCom(new drwMouseCommand(*this));
    return newCom;
  }
...
public:
  ...
  virtual s_ptr Clone() {
    s_ptr newCom(new drwNewSceneCommand(*this));
    return newCom;
  }
...
public:
  ...
  drwSceneParamsCommand(drwSceneParamsCommand &other)
      : m_numberOfFrames(other.m_numberOfFrames) {}
  virtual ~drwSceneParamsCommand() {}
  virtual s_ptr Clone() {
    s_ptr newCom(new drwSceneParamsCommand(*this));
    return newCom;
  }
