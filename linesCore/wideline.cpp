...
WideLine::WideLine(double width)
    : m_width(width), m_pressureWidth(true), m_pressureOpacity(true),
      m_fill(false), m_erase(false), m_prevPoint(0, 0), m_prevPressure(1.0),
...
void WideLine::InternDraw(drwDrawingContext &context) {
  ...
  if (m_fill && m_doneAddingPoints && m_fillIndices.size() > 3) {
    ...
    glDrawElements(GL_TRIANGLE_FAN, m_fillIndices.size(), GL_UNSIGNED_INT,
                   m_fillIndices.GetBuffer());
  ...
  glDrawElements(GL_QUADS, m_indices.size(), GL_UNSIGNED_INT,
                 m_indices.GetBuffer());
  ...
  int xWinMin = 0;
  int yWinMin = 0;
  context.WorldToGLFrame(m_boundingBox.XMin(), m_boundingBox.YMin(), xWinMin,
                         yWinMin);
  int xWinMax = 1;
  int yWinMax = 1;
  context.WorldToGLFrame(m_boundingBox.XMax(), m_boundingBox.YMax(), xWinMax,
                         yWinMax);
