...
public:
  ...
  static void SetPixDampWidth(float pixDampWidth) {
    m_pix_damp_width = pixDampWidth;
  }
  static float GetPixDampWidth() { return m_pix_damp_width; }
  static void SetSigmaLarge(float sigmaLarge) {
    m_sigma_large = sigmaLarge;
    if (sigmaLarge < m_sigma_small)
      m_sigma_small = m_sigma_large;
  }
  static float GetSigmaLarge() { return m_sigma_large; }
  static void SetSigmaSmall(float ss) {
    m_sigma_small = ss;
    if (m_sigma_large < m_sigma_small)
      m_sigma_large = m_sigma_small;
  }
