...
void drwToolbox::AddTool(drwTool *tool) {
  connect(tool, SIGNAL(CommandExecuted(drwCommand::s_ptr)),
          SLOT(NotifyCommandExecuted(drwCommand::s_ptr)));
...
void drwToolbox::GotoPrevFrame() {
  int prevFrame =
      m_currentFrame == 0 ? GetNumberOfFrames() - 1 : m_currentFrame - 1;
  SetCurrentFrame(prevFrame);
}

int drwToolbox::GetNumberOfFrames() { return m_scene->GetNumberOfFrames(); }
...
void drwToolbox::ExecuteCommand(drwCommand::s_ptr command) {
  if (command->GetCommandId() == drwIdSetFrameCommand) {
    drwSetFrameCommand *setFrameCom =
        dynamic_cast<drwSetFrameCommand *>(command.get());
