...
// WARNING: here, window coord are in window-system coordinate space (y=0 at the
// top of the window) and world coordinates are in OpenGL space (y=0 at the
// bottom of the window). WorldToGLWindow (next function) is not the inverse
// operation.
void drwCamera::NativeWindowToWorld(int xwin, int ywin, double &xworld,
                                    double &yworld) {
  xworld = m_winPosWorld[0] + UnitsPerPixel() * xwin;
  yworld = m_winPosWorld[1] + UnitsPerPixel() * (m_winSizePix[1] - ywin - 1);
}

// WARNING: here, window coordinates are in the OpenGL window space (y=0 at the
// bottom of the window)
void drwCamera::GLWindowToWorld(int xwin, int ywin, double &xworld,
                                double &yworld) {
  xworld = m_winPosWorld[0] + UnitsPerPixel() * xwin;
  yworld = m_winPosWorld[1] + UnitsPerPixel() * ywin;
}

void drwCamera::GLWindowToGLFrame(int xwin, int ywin, int &xframe,
                                  int &yframe) {
  xframe = xwin - m_framePosPix[0];
  yframe = ywin - m_framePosPix[1];
}

void drwCamera::WorldToGLWindow(double xworld, double yworld, int &xwin,
                                int &ywin) {
...
// WARNING: here, window coordinates are in the OpenGL window space (y=0 at the
// bottom of the window) this is not the inverse of the previous function
// (WindowToWorld)
void drwCamera::WorldToGLFrame(double xworld, double yworld, int &xwin,
                               int &ywin) {
  double scaleFactor = PixelsPerUnit();
...
void drwCamera::SetupForWindow() {
  ...
  glLoadIdentity();
  gluOrtho2D(m_winPosWorld[0], m_winPosWorld[0] + m_winSizeWorld[0],
             m_winPosWorld[1], m_winPosWorld[1] + m_winSizeWorld[1]);
...
void drwCamera::SetupToRenderTextureToScreen() {
  glViewport(m_framePosPix[0], m_framePosPix[1], m_frameSizePix[0],
             m_frameSizePix[1]);
}
...
void drwCamera::Print() {
  std::cout << "Window size " << m_winSizePix[0] << " x " << m_winSizePix[1]
            << " pix" << std::endl;
  std::cout << "Frame size (world): " << m_frameSizeWorld[0] << " x "
            << m_frameSizeWorld[1] << " units" << std::endl;
  std::cout << "Frame padding: " << m_framePadding << std::endl;
  std::cout << "Frame size (pix): " << m_frameSizePix[0] << " x "
            << m_frameSizePix[1] << std::endl;
  std::cout << "Frame pos (pix): ( " << m_framePosPix[0] << ", "
            << m_framePosPix[1] << " )" << std::endl;
}

void drwCamera::UpdateSizes() {
  // Compute m_frameSizePix : find the rectangle with same proportion as the
  // frame that fits inside the window
  RectThatFitsInside(m_frameSizeWorld[0], m_frameSizeWorld[1], m_framePadding,
                     m_winSizePix[0], m_winSizePix[1], m_frameSizePix[0],
                     m_frameSizePix[1]);
...
void drwCamera::RectThatFitsInside(double w, double h, double percentBorder,
                                   int winW, int winH, int &rectW, int &rectH) {
