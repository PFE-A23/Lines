...
LinesCore::LinesCore() {
  ...
  connect(m_scene, SIGNAL(NumberOfFramesChanged(int)), this,
          SLOT(NumberOfFramesChangedSlot()));
  connect(m_scene, SIGNAL(CommandExecuted(drwCommand::s_ptr)), this,
          SLOT(IncomingLocalCommand(drwCommand::s_ptr)));

  // Local toolbox
  m_localToolbox = new drwToolbox(m_scene, this, true);
  m_toolboxes[m_localToolboxId] = m_localToolbox;
  connect(m_localToolbox, SIGNAL(FrameChanged()), this,
          SLOT(FrameChangedSlot()));
  connect(m_localToolbox, SIGNAL(CommandExecuted(drwCommand::s_ptr)), this,
          SLOT(IncomingLocalCommand(drwCommand::s_ptr)));

  // Command db
  m_commandDb = new drwCommandDatabase(this);

  // do this after everything else is initialized to make sure we generate a
  // command for the db.
  ...
  // Mechanism to exectute net commands (coming from different threads) into the
  // main thread
  connect(this, SIGNAL(NetCommandAvailable()), this,
          SLOT(ProcessPendingNetCommandsSlot()), Qt::QueuedConnection);
}
...
drwLineTool *LinesCore::GetLineTool() {
  drwLineTool *lineTool =
      dynamic_cast<drwLineTool *>(m_localToolbox->GetTool(0));
  Q_ASSERT(lineTool);
  return lineTool;
}
...
bool LinesCore::IsAnimationModified() { return m_commandDb->IsModified(); }
...
void LinesCore::EmitStateCommands() { m_localToolbox->EmitStateCommand(); }
...
void LinesCore::SetRenderSize(int w, int h) { m_renderer->SetRenderSize(w, h); }

void LinesCore::SetShowCursor(bool show) {
  m_localToolbox->SetShowCursor(show);
}

bool LinesCore::GetOnionSkinEnabled() { return m_onionSkinEnabled; }
...
bool LinesCore::IsFlippingFrameModeEnabled() { return m_flipFramesMode; }
...
void LinesCore::WriteSettings(QSettings &s) {
  m_localToolbox->WriteSettings(s);
}

void LinesCore::Render() { m_renderer->Render(); }

void LinesCore::MouseEvent(drwMouseCommand::MouseCommandType commandType,
                           double xWin, double yWin, double pressure, int xTilt,
                           int yTilt, double rotation,
  ...
  if (m_flipFramesMode || m_flippingFrames /* haven't finished*/) {
    ...
    drwCommand::s_ptr command(
        new drwMouseCommand(commandType, xWorld, yWorld, 0.0, xWin, yWin, xTilt,
                            yTilt, pressure, rotation, tangentialPressure));
    m_localToolbox->ExecuteCommand(command);
  }
}

void LinesCore::MouseEventWorld(drwMouseCommand::MouseCommandType type,
                                double xWorld, double yWorld, double pressure) {
  int xWin, yWin;
  m_renderer->WorldToGLWindow(xWorld, yWorld, xWin, yWin);
  drwCommand::s_ptr command(new drwMouseCommand(type, xWorld, yWorld, 0.0, xWin,
                                                yWin, 0, 0, pressure, 0, 0));
  m_localToolbox->ExecuteCommand(command);
}

int LinesCore::GetNumberOfFrames() { return m_scene->GetNumberOfFrames(); }

void LinesCore::SetNumberOfFrames(int nb) { m_scene->SetNumberOfFrames(nb); }

int LinesCore::GetCurrentFrame() { return m_localToolbox->GetCurrentFrame(); }

void LinesCore::SetCurrentFrame(int frame) {
  m_localToolbox->SetCurrentFrame(frame);
}

void LinesCore::NextFrame() { m_localToolbox->GotoNextFrame(); }

void LinesCore::PrevFrame() { m_localToolbox->GotoPrevFrame(); }

void LinesCore::GotoStart() { SetCurrentFrame(0); }

void LinesCore::GotoEnd() { SetCurrentFrame(GetNumberOfFrames() - 1); }
...
bool LinesCore::IsPlaying() { return m_isPlaying; }
...
void LinesCore::NotifyNeedRender() { m_renderer->NeedRedraw(); }

void LinesCore::EnableRendering(bool enable) {
  m_renderer->EnableRendering(enable);
}

void LinesCore::SetRemoteIO(drwRemoteCommandIO *io) { m_remoteIO = io; }

int LinesCore::RequestNewUserId() { return ++m_lastUsedUserId; }

int LinesCore::GetLocalUserId() { return m_localToolboxId; }
...
void LinesCore::LockDb(bool l) { m_commandDb->LockDb(l); }
...
void LinesCore::IncomingDbCommand(drwCommand::s_ptr command) {
  if (command->GetCommandId() == drwIdSceneParamsCommand) {
    drwSceneParamsCommand *serverMsg =
        dynamic_cast<drwSceneParamsCommand *>(command.get());
...
void LinesCore::FrameChangedSlot() { emit PlaybackSettingsChangedSignal(); }
...
void LinesCore::HandleFlipFrameEvent(
    drwMouseCommand::MouseCommandType commandType, double yWin) {
  ...
  if (commandType == drwMouseCommand::Press) {
    ...
    int newFrame =
        m_flipStartFrame - ((int)yWin - m_flipStartY) / nbPixPerFrame;
    if (newFrame >= GetNumberOfFrames())
      newFrame = newFrame % GetNumberOfFrames();
    if (newFrame < 0)
      newFrame =
          GetNumberOfFrames() - (std::abs(newFrame) % GetNumberOfFrames());
    SetCurrentFrame(newFrame);
  }
}
...
void LinesCore::ExecuteOneNetCommand(drwCommand::s_ptr com) {
  if (com->GetCommandId() == drwIdNewSceneCommand) {
    ...
    drwSceneParamsCommand *serverMsg =
        dynamic_cast<drwSceneParamsCommand *>(com.get());
    m_scene->SetNumberOfFrames(serverMsg->GetNumberOfFrames());
