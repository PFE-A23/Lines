...
drwCommand::s_ptr drwCommand::InstanciateSubclass(drwCommandId id) {
  ...
  case drwIdSceneParamsCommand:
    ...
    break;
  default:
    Q_ASSERT(0); // If we get here, we try to read a command type that is not
                 // yet defined.
    break;
  }
  return newCommand;
...
int drwCommand::HeaderSize() { return sizeof(quint32) + sizeof(int); }
...
int drwSetFrameCommand::BodySize() { return sizeof(NewFrame); }

void drwSetFrameCommand::Read(QDataStream &stream) { stream >> NewFrame; }
...
drwLineToolParamsCommand::drwLineToolParamsCommand()
    : Color(Vec4(1.0, 1.0, 1.0, 1.0)), BaseWidth(1.0), PressureWidth(false),
      PressureOpacity(false), Fill(false), Erase(false), Persistence(0) {}

drwLineToolParamsCommand::drwLineToolParamsCommand(
    drwLineToolParamsCommand &other)
    : drwCommand(other), Color(other.Color), BaseWidth(other.BaseWidth),
      PressureWidth(other.PressureWidth),
      PressureOpacity(other.PressureOpacity), Fill(other.Fill),
      Erase(other.Erase), Persistence(other.Persistence) {}

int drwLineToolParamsCommand::BodySize() {
  return (4 * sizeof(double) + sizeof(BaseWidth) + sizeof(PressureWidth) +
          sizeof(PressureOpacity) + sizeof(Fill) + sizeof(Erase) +
          sizeof(Persistence));
}
...
bool drwLineToolParamsCommand::Concatenate(drwCommand *other) {
  if (other->GetCommandId() == drwIdLineToolParamsCommand) {
    drwLineToolParamsCommand *o =
        dynamic_cast<drwLineToolParamsCommand *>(other);
    Q_ASSERT(o);
    Color = o->Color;
...
void drwServerInitialCommand::Write(QTextStream &stream) {
  stream << "ServerInitialCommand: ProtocolVersion = " << ProtocolVersion
...
drwMouseCommand::drwMouseCommand()
    : drwCommand(), m_type(Press), m_x(0.0), m_y(0.0), m_z(0.0), m_xPixel(0),
      m_yPixel(0), m_xTilt(0), m_yTilt(0), m_pressure(1.0), m_rotation(0.0),
      m_tangentialPressure(0.0) {}

drwMouseCommand::drwMouseCommand(drwMouseCommand::MouseCommandType commandType,
                                 double x, double y, double z, int xPix,
                                 int yPix, int xTilt, int yTilt,
                                 double pressure, double rotation,
                                 double tangentialPressure)
    : drwCommand(), m_type(commandType), m_x(x), m_y(y), m_z(z), m_xPixel(xPix),
      m_yPixel(yPix), m_xTilt(xTilt), m_yTilt(yTilt), m_pressure(pressure),
      m_rotation(rotation), m_tangentialPressure(tangentialPressure) {}

drwMouseCommand::drwMouseCommand(drwMouseCommand &other)
    : drwCommand(other), m_type(other.m_type), m_x(other.m_x), m_y(other.m_y),
      m_z(other.m_z), m_xPixel(other.m_xPixel), m_yPixel(other.m_yPixel),
      m_xTilt(other.m_xTilt), m_yTilt(other.m_yTilt),
      m_pressure(other.m_pressure), m_rotation(other.m_rotation),
...
int drwMouseCommand::BodySize() {
  return sizeof(m_type) + sizeof(m_x) + sizeof(m_y) + sizeof(m_z) +
         sizeof(m_xPixel) + sizeof(m_yPixel) + sizeof(m_xTilt) +
         sizeof(m_yTilt) + sizeof(m_pressure) + sizeof(m_rotation) +
         sizeof(m_tangentialPressure);
}
...
void drwMouseCommand::Write(QTextStream &stream) {
  stream << "Pos = (" << m_x << ", " << m_y << ", " << m_z
         << ") Screen Pos = ( " << m_xPixel << ", " << m_yPixel << ") Tilt = ("
         << m_xTilt << ", " << m_yTilt << ")  Pressure = " << m_pressure
         << "   Rotation = " << m_rotation
         << "  Tang pressure = " << m_tangentialPressure;
}
...
int drwNewSceneCommand::BodySize() { return 0; }

void drwNewSceneCommand::Read(QDataStream &stream) {}

bool drwNewSceneCommand::WriteImpl(QDataStream &stream) { return true; }

void drwNewSceneCommand::Write(QTextStream &stream) { stream << "New Scene"; }

//===================================
// drwSceneParamsCommand Implementation
//===================================

int drwSceneParamsCommand::BodySize() { return sizeof(m_numberOfFrames); }
