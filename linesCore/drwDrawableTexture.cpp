...
drwDrawableTexture::drwDrawableTexture()
    : m_internalFormat(GL_RGBA), m_pixelType(GL_RGBA),
      m_componentType(GL_UNSIGNED_BYTE), m_downloadPixelType(GL_RGBA),
      m_isDrawingInTexture(false), m_texId(0), m_fbId(0), m_backupFbId(0),
      m_width(1), m_height(1) {}

drwDrawableTexture::~drwDrawableTexture() { Release(); }

void drwDrawableTexture::SetPixelFormatToRGBU8() {
  m_internalFormat = GL_RGB;
  m_pixelType = GL_RGB;
  m_componentType = GL_UNSIGNED_BYTE;
  m_downloadPixelType = GL_RGB;
}
// void drwDrawableTexture::SetPixelFormatToGreyF16() { m_internalFormat =
// GL_R16F; m_pixelType = GL_LUMINANCE; m_componentType = GL_FLOAT;
// m_downloadPixelType = GL_RED; }
void drwDrawableTexture::SetPixelFormatToGreyF32() {
  m_internalFormat = GL_R32F;
  m_pixelType = GL_RED;
  m_componentType = GL_FLOAT;
  m_downloadPixelType = GL_RED;
}
void drwDrawableTexture::SetPixelFormatToRGBAF32() {
...
void drwDrawableTexture::Resize(int width, int height) {
  ...
  if (!m_texId) {
    ...
    glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, m_internalFormat, width, height,
                 0, m_pixelType, m_componentType, 0);
    ...
    glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT,
                              GL_TEXTURE_RECTANGLE_ARB, m_texId, 0);
...
void drwDrawableTexture::Upload(unsigned char *buffer) {
  glBindTexture(GL_TEXTURE_RECTANGLE_ARB, m_texId);
  glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, m_internalFormat, m_width, m_height,
               0, m_pixelType, GL_UNSIGNED_BYTE, buffer);
  glBindTexture(GL_TEXTURE_RECTANGLE_ARB, 0);
}

void drwDrawableTexture::Download(unsigned char *buffer) {
  glBindTexture(GL_TEXTURE_RECTANGLE_ARB, m_texId);
  glGetTexImage(GL_TEXTURE_RECTANGLE_ARB, 0, m_downloadPixelType,
