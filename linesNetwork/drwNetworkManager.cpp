...
drwNetworkManager::drwNetworkManager()
    : m_lines(0), m_inThread(0), m_state(Idle) {
  // Start the thread
  start();
}
...
bool drwNetworkManager::IsSharing() { return (m_state == Sharing); }
...
bool drwNetworkManager::IsConnected() {
  return (m_state == WaitingForConnection || m_state == ConnectionTimedOut ||
          m_state == Connected);
}
...
void drwNetworkManager::run() {
  ...
  connect(this, SIGNAL(NewCommandsToSendSignal()), m_inThread,
          SLOT(NewCommandsToSend()), Qt::QueuedConnection);
  connect(this, SIGNAL(MessageToThreadSignal()), m_inThread,
          SLOT(MessageToThreadSlot()), Qt::QueuedConnection);
  connect(m_inThread, SIGNAL(MessageFromThreadSignal()), this,
          SLOT(MessageFromThreadSlot()), Qt::QueuedConnection);
...
void drwInThreadAgent::SetConnectAttributes(QString user,
...
void drwInThreadAgent::NewCommandsToSend() {
  ...
  drwNetworkManager::CommandContainer::iterator it =
      m_manager->m_queuedCommands.begin();
...
void drwInThreadAgent::MessageToThreadSlot() {
  if (m_messageToThread == drwNetworkManager::StartSharingMsg) {
    ...
    if (!m_client) {
      m_attributesMutex.lock();
      m_client =
          new drwNetworkClient(m_userName, m_remoteIp, m_manager->m_lines);
      m_attributesMutex.unlock();
      connect(m_client, SIGNAL(StateChanged()), this,
              SLOT(ClientStateChanged()));
      m_client->Connect();
    }
...
void drwInThreadAgent::Reset() {
  if (m_client) {
    disconnect(m_client, SIGNAL(StateChanged()), this,
               SLOT(ClientStateChanged()));
    m_client->Disconnect();
