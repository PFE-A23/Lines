...
drwNetworkServer::drwNetworkServer(LinesCore *lc, QObject *parent)
    : QObject(parent), m_isOn(false), m_broadcastTimer(0), m_broadcastSocket(0),
      m_tcpServer(0), m_lines(lc) {
...
void drwNetworkServer::Start() {
  // Broadcast a first datagram and setup a timer to broadcast at regular
  // intervals
  m_broadcastTimer->start(BroadcastTimeout);
  Broadcast();

  // Setup a TCP server to listen for new connections initiated by peers who
  // received our broadcast message
...
void drwNetworkServer::Stop() {
  if (m_isOn) {
    ...
    while (it != m_connections.end()) {
      ...
      // needed before deleting, otherwise the connection lost callback is
      // called.
      disconnect(connection, SIGNAL(ConnectionLost(drwNetworkConnection *)),
                 this, SLOT(ConnectionLost(drwNetworkConnection *)));
      disconnect(connection, SIGNAL(ConnectionReady(drwNetworkConnection *)),
                 this, SLOT(ConnectionReadySlot(drwNetworkConnection *)));
      disconnect(connection, SIGNAL(CommandReceived(drwCommand::s_ptr)), this,
                 SLOT(CommandReceivedSlot(drwCommand::s_ptr)));
...
void drwNetworkServer::GetConnectionUserNamesAndIps(QStringList &userNameList,
                                                    QStringList &ipList) {
  ConnectionsContainer::iterator it = m_connections.begin();
  while (it != m_connections.end()) {
    drwNetworkConnection *connection = it.key();
...
void drwNetworkServer::Broadcast() {
  ...
  m_broadcastSocket->writeDatagram(datagram.data(), datagram.size(),
...
void drwNetworkServer::NewIncomingConnection() {
  while (m_tcpServer->hasPendingConnections()) {
    ...
    connect(connection, SIGNAL(ConnectionLost(drwNetworkConnection *)), this,
            SLOT(ConnectionLost(drwNetworkConnection *)));
    connect(connection, SIGNAL(ConnectionReady(drwNetworkConnection *)), this,
            SLOT(ConnectionReadySlot(drwNetworkConnection *)));
    connect(connection, SIGNAL(CommandReceived(drwCommand::s_ptr)), this,
            SLOT(CommandReceivedSlot(drwCommand::s_ptr)), Qt::DirectConnection);
...
void drwNetworkServer::ConnectionReadySlot(drwNetworkConnection *connection) {
  ...
  drwCommand::s_ptr command(
      new drwServerInitialCommand(m_lines->GetNumberOfDbCommands()));
...
void drwNetworkServer::CommandReceivedSlot(drwCommand::s_ptr com) {
  drwNetworkConnection *commandSender =
      dynamic_cast<drwNetworkConnection *>(sender());
