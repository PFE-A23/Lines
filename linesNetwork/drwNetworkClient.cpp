...
drwNetworkClient::drwNetworkClient(QString &peerUserName,
                                   QHostAddress &peerAddress, LinesCore *lc)
    : m_connection(0), m_lines(lc), m_peerUserName(peerUserName),
      m_peerAddress(peerAddress), m_numberOfCommandsToRead(0),
      m_totalNumberOfCommandsToRead(0), m_state(Init), m_timerId(-1) {}
...
void drwNetworkClient::Connect() {
  ...
  connect(m_connection, SIGNAL(ConnectionLost(drwNetworkConnection *, QString)),
          this, SLOT(ConnectionLostSlot(drwNetworkConnection *, QString)));
  connect(m_connection, SIGNAL(ConnectionReady(drwNetworkConnection *)), this,
...
void drwNetworkClient::CommandReceivedSlot(drwCommand::s_ptr command) {
  if (m_state == WaitingForNbCommands) {
    if (command->GetCommandId() == drwIdServerInitialCommand) {
      drwServerInitialCommand *serverMsg =
          dynamic_cast<drwServerInitialCommand *>(command.get());
