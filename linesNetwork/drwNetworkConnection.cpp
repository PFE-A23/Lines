...
drwNetworkConnection::drwNetworkConnection(QTcpSocket *socket, QObject *parent)
    : QObject(parent), m_socket(socket), m_peerAddress(socket->peerAddress()),
  ...
  m_socket->setSocketOption(
      ...
      1); // make sure commands are not buffered before sending
...
// client sends username and starts waiting for commands as soon as the server
// has accepted request to connect
drwNetworkConnection::drwNetworkConnection(QString peerUserName,
                                           QHostAddress &address,
                                           QObject *parent)
    : QObject(parent), m_socket(0), m_peerUserName(peerUserName),
...
drwNetworkConnection::~drwNetworkConnection() { Disconnect(); }
