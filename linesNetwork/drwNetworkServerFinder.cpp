...
drwNetworkServerFinder::drwNetworkServerFinder() {
  ...
  connect(m_broadcastSocket, SIGNAL(readyRead()), this,
          SLOT(ProcessReceivedBroadcastMessages()));
}

drwNetworkServerFinder::~drwNetworkServerFinder() { delete m_broadcastSocket; }
...
void drwNetworkServerFinder::ProcessReceivedBroadcastMessages() {
  while (m_broadcastSocket->hasPendingDatagrams()) {
    ...
    quint16 port;
    m_broadcastSocket->readDatagram(datagram.data(), datagram.size(), &address,
                                    &port);
...
void drwNetworkServerFinder::TryAddingServer(QString &user,
                                             QHostAddress &address) {
